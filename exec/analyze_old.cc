...
void analyze_event(MOD::Event & event_being_read, ofstream & output_file, int & event_serial_number, unordered_map<int, pair <int, double>> &lumi_info_by_lumi_block);
...
int main(int argc, char * argv[]) {
    ...
    while( event_being_read.read_event(data_file) && ( event_serial_number <= number_of_events_to_process ) ) {
        ...
        analyze_event(event_being_read, output_file, event_serial_number,lumi_info_by_lumi_block );
...
void analyze_event(MOD::Event & event_being_read, ofstream & output_file, int & event_serial_number, unordered_map<int, pair <int, double>> &lumi_info_by_lumi_block) {
  ...
  // cout << "Event number is " << event_being_read.event_number() << endl;
   ...
   properties.push_back(MOD::Property("crosssection", 1.0/event_being_read.condition().integrated_delivered_lumi()));
...
unordered_map<int, pair <int, double>> get_lumi_info_lumi_block(string & input_file, int number_of_events_to_process) {
    ...
    while( event_being_read.read_event(data_file) && ( event_serial_number <= number_of_events_to_process ) ) {
        ...
        if( (event_serial_number % 1000) == 0 ){
            cout << "Processing event number2 " << event_serial_number << endl;
        ...
        double avg_inst_lumi = event_being_read.condition().integrated_delivered_lumi();
        ...
        if(it != lumi_block_lumi_info.end()){
            ...
            current_pair.second += avg_inst_lumi;
            it->second = current_pair;
        }
        else{
            pair <int, double> new_pair;
            new_pair = make_pair(1, avg_inst_lumi);
