...
int main(int argc, char * argv[]) {
   ...
   while( event_being_read.read_event(data_file) && ( event_serial_number <= number_of_events_to_process ) ) {
       ...
       cout << "Processing event number " << event_serial_number << number_of_events_to_process<< endl;
      ...
      if (event_being_read.assigned_trigger_fired() and (lumi_block_lumi_info.find(hash_value) != lumi_block_lumi_info.end())) { analyze_event(event_being_read, output_file, event_serial_number);}
       for (int i = 0; i < 10; i++ ){
           ...
           if (event_being_read.trigger_exists(trigger_name) and (lumi_block_lumi_info.find(hash_value) != lumi_block_lumi_info.end()) ){
               ...
               if (checked_lumi_blocks.find(to_string(hash_value)+trigger_name) == checked_lumi_blocks.end()){
                   recorded_luminosities[trigger_name] += lumi_block_lumi_info[hash_value].second/trigger_current_prescale*1.0;
   ...
   ofstream effective_luminosity_output("/Users/prekshanaik/Documents/MEng_Project/11MODAnalyzer-master/effective_luminosity_by_jet.csv", ios::out | ios::app);
   ...
   while (it != recorded_luminosities.end())
    {
        ...
        effective_luminosity_output << trigger_name << "," << luminosity;
   ...
   // effective_luminosity_output << "HLT_Jet30 " << total_luminosity_30 << endl;

    
   /*
   ofstream effective_luminosity_output("/Users/prekshanaik/Documents/MEng_Project/11MODAnalyzer-master/effective_luminosity_by_jet.dat", ios::out | ios::app);
