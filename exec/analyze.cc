...
int main(int argc, char * argv[]) {
   ...
   if (data_set_to_process == "sim"){
       ...
       while( event_being_read.read_event(data_file_pfc, type_of_data) && ( event_serial_number_pfc <= number_of_events_to_process ) ) {
           ...
           if (event_being_read.assigned_trigger_fired() and event_being_read.is_trigger_jet_matched() and event_being_read.is_jet_quality_met(number_jets_to_analyze)){
               analyze_event(event_being_read, output_file_pfc, event_serial_number_pfc, type_of_data, number_jets_to_analyze);
           }
           event_being_read = MOD::Event();
       ...
       output_event_counts << get_just_file_name(output_file_gen_decay_name) << "," << event_serial_number_gen -1  << endl;
...
void analyze_event(MOD::Event & event_being_read, ofstream & output_file, int & event_serial_number, string & data_set_to_process, int & number_jets_to_analyze) {
  ...
  if ((event_being_read.data_source() == 3) || (event_being_read.data_source() == 0))
   ...
   //cout << "hardest_eta" << event_being_read.hardest_jet().eta();
