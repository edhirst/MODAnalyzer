...
void analyze_event(MOD::Event & event_being_read, ofstream & output_file, int & event_serial_number, string & data_set_to_process, int & number_jets_to_analyze);
...
int main(int argc, char * argv[]) {
   ...
   if (data_set_to_process == "sim"){
       ...
       MOD::Event event_being_read_pfc;
       MOD::Event event_being_read_gen;


       string output_file_pfc_and_gen_name = replace_string(output_file_name, ".dat", "_sim_pfc_gen.dat");
       ofstream output_file_pfc_and_gen(output_file_pfc_and_gen_name, ios::out);
       ifstream data_file_pfc(argv[1]);
       string type_of_data_pfc = "sim_pfc";
       ifstream data_file_gen(argv[1]);
       string type_of_data_gen = "sim_gen";

       while( event_being_read_pfc.read_event(data_file_pfc, type_of_data_pfc) && event_being_read_gen.read_event(data_file_gen, type_of_data_gen) && ( event_serial_number_pfc <= number_of_events_to_process ) ) {

           if( (event_serial_number_gen % 500) == 0 ){
               cout << "Processing event number " << event_serial_number_pfc << endl;
           }
           
           if (event_being_read_pfc.assigned_trigger_fired() and event_being_read_pfc.is_trigger_jet_matched() and event_being_read_pfc.is_jet_quality_met(number_jets_to_analyze)){
               
               analyze_event(event_being_read_pfc, output_file_pfc_and_gen, event_serial_number_pfc, type_of_data_pfc, number_jets_to_analyze);
               
               MOD::Event decayed_event;
               
               decayed_event.decay_particles(event_being_read_gen, &pythia);

               analyze_event(decayed_event, output_file_pfc_and_gen, event_serial_number_gen, type_of_data_gen, number_jets_to_analyze);
               event_serial_number_pfc++;

           }
           event_being_read_pfc = MOD::Event();
           event_being_read_gen = MOD::Event();
       ...
       string output_file_gen_decay_name = replace_string(output_file_name, ".dat", "_sim_gen_decay.dat");
       ofstream output_file_gen_decay(output_file_gen_decay_name, ios::out);
       ifstream data_file_gen(argv[1]);
       ...
       */
       ...
       output_event_counts << get_just_file_name(output_file_pfc_and_gen_name) << "," << event_serial_number_pfc -1  << endl;
       output_event_counts << get_just_file_name(output_file_pfc_and_gen_name) << "," << event_serial_number_gen -1  << endl;
       output_event_counts << get_just_file_name(output_file_pfc_and_gen_name) << "," << event_serial_number_gen -1  << endl;
...
void analyze_event(MOD::Event & event_being_read, ofstream & output_file, int & event_serial_number, string & data_set_to_process, int & number_jets_to_analyze) {
   ...
   // Don't want pfc cut currently so commented out
  ...
  if ((event_being_read.data_source() == 3) || (event_being_read.data_source() == 0))
   ...
   if(data_set_to_process == "sim_pfc"){
   ...
   if (number_jets_to_analyze >= 2){
       
       double second_hardest_jec = 1.0;
       
       if ((event_being_read.data_source() == 3) || (event_being_read.data_source() == 0))
           second_hardest_jec = event_being_read.get_second_hardest_jet_jec();

       
       vector<PseudoJet> second_hardest_jet_constituents = event_being_read.second_hardest_jet().constituents();
       
       ClusterSequence cs_second_hardest = ClusterSequence(second_hardest_jet_constituents, jet_def_cambridge);
       
       
       if (cs_second_hardest.inclusive_jets().size() == 0) {
       ...
       PseudoJet uncorrected_second_hardest_jet_with_softkiller = cs_second_hardest.inclusive_jets()[0];
       
       
       
       PseudoJet second_hardest_jet = uncorrected_second_hardest_jet_with_softkiller;

       
       
       properties.push_back( MOD::Property("second_hardest_eta", event_being_read.second_hardest_jet().eta()) );
       
       properties.push_back( MOD::Property("second_hardest_pT", second_hardest_jec*event_being_read.second_hardest_jet().pt()) );
       
       properties.push_back( MOD::Property("second_hardest_phi", event_being_read.second_hardest_jet().phi()) );
       
       properties.push_back( MOD::Property("second_hardest_mass_pre_SD", second_hardest_jet.m()) );
       
       properties.push_back( MOD::Property("second_hardest_mul_pre_SD", (int) second_hardest_jet_constituents.size()) );

       
       properties.push_back( MOD::Property("delta_R_y_second_and_first", second_hardest_jet.delta_R(hardest_jet)) );
       ...
       if ((event_being_read.data_source() == 3) || (event_being_read.data_source() == 0))
           third_hardest_jec = event_being_read.get_third_hardest_jet_jec();
       
       
       vector<PseudoJet> third_hardest_jet_constituents = event_being_read.third_hardest_jet().constituents();
       
       ClusterSequence cs_third_hardest = ClusterSequence(third_hardest_jet_constituents, jet_def_cambridge);
       

       
       
       if (cs_third_hardest.inclusive_jets().size() == 0) {
       ...
       PseudoJet uncorrected_third_hardest_jet_with_softkiller = cs_third_hardest.inclusive_jets()[0];
       
       
       
       PseudoJet third_hardest_jet = uncorrected_third_hardest_jet_with_softkiller;
       

       
       properties.push_back( MOD::Property("third_hardest_eta", event_being_read.third_hardest_jet().eta()) );
       
       properties.push_back( MOD::Property("third_hardest_pT", third_hardest_jec*event_being_read.third_hardest_jet().pt()) );
       
       properties.push_back( MOD::Property("third_hardest_phi", event_being_read.third_hardest_jet().phi()) );
       

       
       properties.push_back( MOD::Property("third_hardest_mass_pre_SD", third_hardest_jet.m()) );
       
       properties.push_back( MOD::Property("third_hardest_mul_pre_SD",  (int) third_hardest_jet_constituents.size()) );


       
       properties.push_back( MOD::Property("delta_R_y_second_and_third", second_hardest_jet.delta_R(third_hardest_jet)) );
       
       

       properties.push_back( MOD::Property("delta_phi_second_and_third", second_hardest_jet.delta_phi_to(third_hardest_jet)) );
       
       properties.push_back( MOD::Property("delta_R_y_first_and_third", hardest_jet.delta_R(third_hardest_jet)) );

       
       properties.push_back( MOD::Property("delta_phi_first_and_third", hardest_jet.delta_phi_to(third_hardest_jet)) );
       
       vector<fastjet::PseudoJet> third_track_constituents = MOD::filter_charged(event_being_read.third_hardest_jet().constituents());
   ...
   if (event_serial_number == 1 and data_set_to_process == "sim_pfc") {
      for (unsigned p = 0; p < properties.size(); p++) {
         if (p > 0)
            output_file << setw(padding);

         output_file << properties[p].name() + "_sim_pfc";
      }
      for (unsigned p = 0; p < properties.size(); p++) {
           output_file << setw(padding);
          
          if (p == 0){
              output_file << "Entry_sim_gen";
          }
          else{
           output_file << properties[p].name() + "_sim_gen";
          }
      }
   ...
   for (unsigned q = 0; q < properties.size(); q++) {
      if (not(q == 0 and data_set_to_process == "sim_pfc"))
         output_file << setw(padding);
